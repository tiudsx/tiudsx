...
if (isset($_FILES['upload'])) {
  $totalFiles = count($_FILES['upload']['name']); // 업로드된 파일 수

  for ($i = 0; $i < $totalFiles; $i++) {
    // 각 파일의 정보
    $fileTmpPath = $_FILES['upload']['tmp_name'][$i]; // 임시 파일 경로
    $fileName = $_FILES['upload']['name'][$i]; // 원본 파일 이름
    $fileSize = $_FILES['upload']['size'][$i]; // 파일 크기
    $fileError = $_FILES['upload']['error'][$i];
    $fileType = $_FILES['upload']['type'][$i]; // 파일 MIME 타입
