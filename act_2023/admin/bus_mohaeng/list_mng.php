...
$select_query_bus = "SELECT a.seq, shopname, res_busnum, res_confirm, COUNT(*) AS CntBus,
						(CASE WHEN LEFT(res_busnum, 3) = 'YSa'
								...
								ELSE res_busnum END) AS orderby FROM `AT_RES_SUB` a
									INNER JOIN AT_COUPON_CODE b
										ON a.res_coupon = b.coupon_code
										AND b.couponseq = 31
						WHERE code = 'bus'
							AND res_date = '$selDate' 
							AND res_confirm = 3 
						GROUP BY a.seq, shopname, 
	...
	<tr>
		...
		<td>
			<?foreach($arrBusE as $key=>$value){?>
				<input type="button" name="buspoint" class="bd_btn" busgubun="<?=$key?>" style="padding-top:4px;font-family: gulim,Tahoma,Arial,Sans-serif;" value="<?=fnBusNum($key)?> [<?=$value?>명]" onclick="fnDayList('<?=$key?>', this, 'bus_mohaeng');" />
			<?}?>
		</td>
	</tr>
	<tr>
