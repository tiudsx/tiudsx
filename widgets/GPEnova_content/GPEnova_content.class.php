<?php
/**
 * 클래스명:class GPEnova_content
 * 제작:그레이브디자인(simpleeye) 
 * 메일:simpleeye79@naver.com
 **/
class GPEnova_content extends WidgetHandler {
	/**
	...
	function proc($args) {
		...
		if(!$args->subject_cut_size_m480) $args->subject_cut_size_m480 = 45;//리스트
		if(!$args->subject_cut_size_m) $args->subject_cut_size_m = 37;
		if(!$args->subject_cut_size_m_g480) $args->subject_cut_size_m_g480 = 20;//갤러리
		if(!$args->subject_cut_size_m_g) $args->subject_cut_size_m_g = 17;
		if(!$args->subject_cut_size_m_w480) $args->subject_cut_size_m_w480 = 34;//웹진
		if(!$args->content_cut_size) $args->content_cut_size = 192;
		if(!$args->duration_new) $args->duration_new = 12;
		if(!$args->thumbnail_type) $args->thumbnail_type = 'crop'; 
		if(!$args->thumbnail_width) $args->thumbnail_width = 175; 
		...
		else
		{
			...
			if(!$args->module_srls)
			{
				...
				$output = executeQueryArray('widgets.GPEnova_content.getMids', $obj);
				if($output->data)
				{
...
/**
 * @brief Get a list of comments and return GPEnova_contentItem
 */
	function _getCommentItems($args)
	{
		...
		foreach($output as $key => $oComment)
		{
			...
			$content_item = new GPEnova_contentItem($browser_title);
	...
	function _getDocumentItems($args)
	{
		...
		$obj = new stdClass();
		$obj->module_srl = $args->module_srl;
		$output = executeQueryArray('widgets.GPEnova_content.getCategories',$obj);
		...
		$obj->list_count = $args->list_count * $args->page_count;
		if(!$args->show_secret){$obj->statusList = array('PUBLIC');} //비밀글 출력유무
		$output = executeQueryArray('widgets.GPEnova_content.getNewestDocuments', $obj);
		...
		if(count($output->data))
		{
			...
			for($i=0,$c=count($document_srls);$i<$c;$i++)
			{
				...
				$content_item = new GPEnova_contentItem( $args->module_srls_info[$module_srl]->browser_title );
	...
	function _getRssItems($args)
	{
		...
		if($xml_doc->rss)
		{
			...
			foreach ($items as $key => $value)
			{
				...
				$content_item = new GPEnova_contentItem($rss->title);
	...
	function _compile($args,$content_items)
	{
		...
		$widget_info->subject_cut_size_m480 = $args->subject_cut_size_m480;
		$widget_info->subject_cut_size_m = $args->subject_cut_size_m;
		$widget_info->subject_cut_size_m_g480 = $args->subject_cut_size_m_g480;
		$widget_info->subject_cut_size_m_g = $args->subject_cut_size_m_g;
		$widget_info->subject_cut_size_m_w480 = $args->subject_cut_size_m_w480;
		...
		$widget_info->gall_wg_padding = $args->gall_wg_padding;
		$widget_info->gall_wg_botpadding = $args->gall_wg_botpadding;
		$widget_info->gall_wg_toppadding = $args->gall_wg_toppadding;
		$widget_info->gall_wg_comment = $args->gall_wg_comment;
		$widget_info->gall_wg_comm_correc = $args->gall_wg_comm_correc;
		...
		$widget_info->list_con_se = $args->list_con_se;
		$widget_info->tab_type = $args->tab_type;
		$widget_info->tab_color = $args->tab_color;
		$widget_info->tap_wg_width = $args->tap_wg_width;
		$widget_info->markup_type = $args->markup_type;
		$widget_info->show_secret = $args->show_secret;
		$widget_info->wg_height = $args->wg_height;
...
class GPEnova_contentItem extends Object
{
	...
	var $domain = null;
	//set
	function GPEnova_contentItem($browser_title=''){$this->browser_title = $browser_title;}
	function setContentsLink($link){$this->contents_link = $link;}
	...
	function setExtraImages($extra_images){$this->add('extra_images',$extra_images);}
	...
	function setLink($url){$this->add('url',$url);}
	function setTitle($title){$this->add('title',$title);}
	function setThumbnail($thumbnail){$this->add('thumbnail',$thumbnail);}
	function setContent($content){$this->add('content',$content);}
	function setRegdate($regdate){$this->add('regdate',$regdate);}
	function setNickName($nick_name){$this->add('nick_name',$nick_name);}
	function setAuthorSite($site_url){$this->add('author_site',$site_url);}
	function setCategory($category){$this->add('category',$category);}
	//get
	function getBrowserTitle(){return $this->browser_title;}
	function getDomain(){return $this->domain;}
	function getContentsLink(){return $this->contents_link;}
	function getFirstThumbnailIdx(){return $this->first_thumbnail_idx;}
	function getLink(){return $this->get('url');}
	function getModuleSrl(){return $this->get('module_srl');}
	...
	function getContent(){return $this->get('content');}
	function getCategory(){return $this->get('category');}
	function getNickName(){return $this->get('nick_name');}
	function getAuthorSite(){return $this->get('author_site');}
	...
	function printExtraImages(){return $this->get('extra_images');}
	function haveFirstThumbnail(){return $this->has_first_thumbnail_idx;}
	function getThumbnail(){return $this->get('thumbnail');}
	function getMemberSrl(){return $this->get('member_srl');}
}