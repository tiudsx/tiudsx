...
$select_query = "SELECT A.*, REPLACE(B.name, '서핑버스 ', '') AS name FROM `AT_COUPON_CODE` AS A 
                    ...
                    WHERE A.couponseq IN (18, 19) AND A.use_yn = 'N'
...
<?while ($row = mysqli_fetch_assoc($result_setlist)){
    ...
    ?>
        <tr>
            ...
            <td><input type="button" class="gg_btn res_btn_color2" style="width:40px; height:25px;" value="삭제" onclick="fnBusCouponDel(<?=$row['codeseq']?>);" /></td>
            <td><?=$rtnText?></td>
            <td>
                <?=$rtnTextCode?>                
            </td>
